/*
 * $Id: hbgtcore.c 13577 2010-01-14 17:59:30Z vszakats $
 */
                                  ...
                                  int iColor, BYTE bAttr, USHORT usChar )
{
   ...
   if( HB_GTSELF_CHECKPOS( pGT, iRow, iCol, &lIndex ) )
   {
      ...
      // Para forçar a atualizacao de toda area modificada para nao ficar buracos na GT
      pGT->prevBuffer[ lIndex ].c.usChar = usChar-1;
      pGT->prevBuffer[ lIndex ].c.bColor = ( BYTE ) iColor-1;
      pGT->prevBuffer[ lIndex ].c.bAttr  = bAttr-1;
...
static int hb_gt_def_MouseReadKey( PHB_GT pGT, int iEventMask )
{
   ...
   if( HB_GTSELF_MOUSEISPRESENT( pGT ) )
   {
      if( iEventMask & INKEY_LDOWN && HB_GTSELF_MOUSEBUTTONPRESSED( pGT, 0, &iRow, &iCol ) )
      {
         ...
         if( timer - pGT->nMouseLeftTimer <= ( HB_ULONG ) HB_GTSELF_MOUSEGETDOUBLECLICKSPEED( pGT ) )
            iKey = K_LDBLCLK;
         else
            iKey = K_LBUTTONDOWN;
      ...
      {
         HB_ULONG timer = hb_dateMilliSeconds();
         if( timer - pGT->nMouseRightTimer <= ( HB_ULONG ) HB_GTSELF_MOUSEGETDOUBLECLICKSPEED( pGT ) )
            iKey = K_RDBLCLK;
         else
            iKey = K_RBUTTONDOWN;
      ...
      {
         HB_ULONG timer = hb_dateMilliSeconds();
         if( timer - pGT->nMouseMiddleTimer <= ( HB_ULONG ) HB_GTSELF_MOUSEGETDOUBLECLICKSPEED( pGT ) )
            iKey = K_MDBLCLK;
         else
            iKey = K_MBUTTONDOWN;
