...
static int bits = 32;
static int num_col = 800;
static int num_lin = 600;
static int full_col = 800;
static int full_lin = 600;
static int lExpand = 0;
static int nModo = GFX_AUTODETECT_WINDOWED;
...
static AL_BITMAP * s_buffer = NULL;
...
static void hb_gt_alleg_DoCursor( int iRow, int iCol, int iStyle )
{
                               ...
                               &iTop, &iLeft, &iBottom, &iRight ) )
   {
      ...
      if (lExpand)
         {
            al_blit( s_bmp, s_buffer, iLeft, iTop, iLeft, iTop, iRight - iLeft + 1, iBottom - iTop + 1 );
         }
         else
         {
                               ...
                               &iTop, &iLeft, &iBottom, &iRight ) )
   {
      ...
      if (lExpand)
         {
            al_draw_rect_fill( s_buffer, iLeft, iTop, iRight, iBottom, s_pClr[ 7 ] );
         }
      else
         {
...
static void hb_gt_alleg_ScreenUpdate( PHB_GT pGT )
{
   ...
   if( fPix || fCursor )
   {
      ...
      if( fPix )
      {
         ...
         if (lExpand)
            {
               al_blit( s_bmp, s_buffer, s_GFXUpd.iLeft, s_GFXUpd.iTop,
      ...
      if (lExpand)
               stretch_blit(s_buffer, al_screen, 0, 0, s_buffer->w, s_buffer->h,
                            0, 0, full_col, full_lin);
...
//         iRet = al_set_gfx_mode( AL_GFX_AUTODETECT, iWidth, iHeight, 0, 0 );
         ...
         //fprintf(stdout,"{ %d - %d - Expand: %d}",full_col,full_lin,lExpand);
         iRet = al_set_gfx_mode( nModo, full_col, full_lin, 0, 0 );
//      }
//      if( iRet != 0 )
//      {
...
//      }
      ...
      s_mouseBound.iTop    = 0;
      s_mouseBound.iRight  = num_col -1; //AL_SCREEN_W - 1;
      s_mouseBound.iBottom = num_lin - 1; //AL_SCREEN_H - 1;
      s_mouseButtonsState  = al_mouse_b;
      if (lExpand)
         {
            al_show_mouse( s_buffer );
         }
      else
         {
...
//         ixFP = ( HB_BYTE ) ( AL_SCREEN_W / s_iScrWidth ) * 2;
      if( num_col != s_byFontWidth * s_iScrWidth )
         ixFP = ( HB_BYTE ) ( num_col / s_iScrWidth ) * 2;

//      if( AL_SCREEN_H != s_byFontSize * s_iScrHeight )
//      {
//         iyFP = ( HB_BYTE ) ( AL_SCREEN_H / s_iScrHeight );
      if( num_lin != s_byFontSize * s_iScrHeight )
      {
         iyFP = ( HB_BYTE ) ( num_lin / s_iScrHeight );
         if( iyFP & 1 )
            iyFP--;
      }
      ...
      s_CLIP.iTop      = 0;
      s_CLIP.iLeft     = 0;
      s_CLIP.iBottom   = num_lin -1; //AL_SCREEN_H - 1;
      s_CLIP.iRight    = num_col - 1; //AL_SCREEN_W - 1;
      s_iCurCol        = 0;
      s_iCurRow        = 0;
...
//      s_bmp = al_create_system_bitmap( AL_SCREEN_W, AL_SCREEN_H );
      if (lExpand)
      {
         s_buffer = al_create_system_bitmap( num_col, num_lin );
         if( ! s_buffer )
         {
            //s_bmp = al_create_bitmap( AL_SCREEN_W, AL_SCREEN_H );
            s_buffer = al_create_bitmap( num_col, num_lin );
            if( ! s_buffer )
               hb_gt_alleg_Error( "Could not allocate double buffer bitmap" );
         }
      }
      s_bmp = al_create_system_bitmap( num_col, num_lin );
      if( ! s_bmp )
      {
         //s_bmp = al_create_bitmap( AL_SCREEN_W, AL_SCREEN_H );
         s_bmp = al_create_bitmap( num_col, num_lin );
         if( ! s_bmp )
            hb_gt_alleg_Error( "Could not allocate double buffer bitmap" );
      }
      ...
      if( ! lClearInit )
      {
         ...
         if (lExpand)
         {
            al_clear_to_color( s_buffer, iColor );
         }
         else
         {
...
HB_FUNC(_GET_BUFFER)
{
   if (lExpand)
     {
        hb_retnl((HB_ULONG) s_buffer);
     }
   else
     {
        hb_retnl((HB_ULONG) al_screen);
     }
}
...
HB_FUNC(_CONFIG_LIB)
{
   ...
   if ( full_col<num_col ) { full_col=num_col; };
   if ( full_lin<num_lin ) { full_lin=num_lin; };

   if ((full_col==num_col) && (full_lin==num_lin))
      {
         lExpand=0;
      }else
      {
...
HB_FUNC(__CONFIG_DRIVER)
{
   nModo=AL_ID(hb_parni(1),hb_parni(2),hb_parni(3),hb_parni(4));
}
...
void alleg_present_bitmap( BITMAP *src, int dx, int dy, int dw, int dh )
{
   ...
   if( s_bmp != NULL )
      stretch_blit( src, s_bmp, 0, 0, src->w, src->h, dx, dy, dw, dh );

   if( lExpand && s_buffer != NULL )
      stretch_blit( src, s_buffer, 0, 0, src->w, src->h, dx, dy, dw, dh );
   ...
   if( lExpand && s_buffer != NULL )
      ...
      al_blit( s_bmp, al_screen, 0, 0, 0, 0, num_col, num_lin );
   else
      stretch_blit( src, al_screen, 0, 0, src->w, src->h, dx, dy, dw, dh );
   al_release_screen();
   al_unscare_mouse();
}
...
{ 
   int iTop=hb_parni(1);
   int iLeft=hb_parni(2);
   const char * cBuf=hb_parc(3);
   int iColor=hb_parni(4);

   scare_mouse();
   if (lExpand)
      {
         ssfDrawText( s_buffer, &s_ssfDefaultFont, cBuf, iLeft, iTop, iColor );
      }
   else
      {
         ssfDrawText( al_screen, &s_ssfDefaultFont, cBuf, iLeft, iTop, iColor );
      }
   unscare_mouse();
}
...
//                                        AL_SCREEN_W : s_byFontWidth * s_iScrWidth );
         pInfo->pResult = hb_itemPutNI( pInfo->pResult, s_fInit ?
                                        num_col : s_byFontWidth * s_iScrWidth );
...
//                                        AL_SCREEN_H : s_byFontSize * s_iScrHeight );
         pInfo->pResult = hb_itemPutNI( pInfo->pResult, s_fInit ?
                                        num_lin : s_byFontSize * s_iScrHeight );
...
HB_FUNC( ALLEGGAMEVIDEO )
{
   s_fGameVideo = hb_parl( 1 ) ? 1 : 0;
   if( s_fGameVideo )
      al_show_mouse( NULL );
}
